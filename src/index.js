...
function MyWaterfall(user,dir,colNum){ // console.log("waterfall:"); 
  // 测试用技术器 // 调用瀑布流 
  var n = 0; var m = 0; 
      ...
      $.each(imgItems, 
          function(i, item){
            ...
            res.push(
              ...
              '<a onclick="togglePhotoView('+item.id+');return false;" href="photo.php?user='+user+'&dir='+dir+'&id='+item.id+'&img='+item.href+'">'+  //target="__blank'+i
              '<img  width="'+310+'" height="'+(item.height/item.width*310+rand(30))+'" oncontextmenu="$(this).contextMenu(\'menu\',menuAdapter);" src="'+item.src+'" data-pinit="registered">'+
              '</a> '+
              ...
              (item.desp!=""?item.desp+
                '<button onclick="changeDespForm(this'+',\''+user+'\',\''+dir+'\',\''+item.href+'\',\''+item.desp+'\',\''+item.ref+'\');" style="background:transparent;border:0;color:#aaa">修改</button>'
                :'<button onclick="showDespForm(this'+',\''+user+'\',\''+dir+'\',\''+item.href+'\');" style="background:transparent;border:1px solid rgba(220,220,220,0.05);color:rgba(160,160,160,0.5)">添加描述......</button>')+'</div>'+
  ...
  this.wf = new Waterfall(
      { container: $('#container'), colWidth: 310, maxCol: colNum,preDistance: 0,
        load: function(){
          ...
          $.get('more_js.php?m='+m+"&user="+user+"&dir="+dir, function(data) {
...
function delImg(element,img){
  ...
  $.ajax({
    url:"img_js.php", 
    data:"action=del&user="+user+"&dir="+curDir+"&img="+img, type:'post', dataType:'text', 
